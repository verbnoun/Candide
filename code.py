...
class SPIReceiver:
    """Handles SPI slave communication and message processing"""
    # Message Types (shared between Bartleby and Candide)
    HELLO_BART = 0xA0
    HI_CANDIDE = 0xA1
    
    def __init__(self, clock_pin, mosi_pin, miso_pin, cs_pin, detect_pin):
        print("Initializing SPI Receiver...")
        # Configure SPI as slave
        self.spi = busio.SPI(clock=clock_pin, MOSI=mosi_pin, MISO=miso_pin)
        ...
        self.detect = digitalio.DigitalInOut(detect_pin)
        self.detect.direction = digitalio.Direction.INPUT
        self.detect.pull = digitalio.Pull.UP
        
        # State tracking
        self.handshake_complete = False
        self.last_message = None
        
        print(f"SPI Pins: CLK={clock_pin}, MOSI={mosi_pin}, MISO={miso_pin}, CS={cs_pin}, DETECT={detect_pin}")

    def check_controller(self):
        """Check for controller presence and initiate handshake"""
        if not self.detect.value and not self.handshake_complete:
            print("Controller detected! Sending HELLO_BART")
            self.send_hello()
            return True
        return False

    def send_hello(self):
        """Send initial greeting to controller"""
        while not self.spi.try_lock():
            pass
        try:
            self.cs.value = False
            message = bytearray([self.HELLO_BART, 0, 0, 0])
            self.spi.write(message)
            print("Sent: HELLO_BART")
        finally:
            self.spi.unlock()
            self.cs.value = True

    def read_message(self):
        """Read a 4-byte message if available"""
        if not self.cs.value:  # CS is active low
            try:
                ...
                try:
                    ...
                    if buffer[0] == self.HI_CANDIDE and not self.handshake_complete:
                        print("Controller says: HI_CANDIDE")
                        self.handshake_complete = True
                        print("Handshake complete! Ready for MIDI")
                        return None
                    
                    # Only pass through MIDI messages after handshake
                    if self.handshake_complete:
                        print(f"SPI IN: [{hex(buffer[0])} {buffer[1]} {buffer[2]} {buffer[3]}]")
                        return buffer
    ...
    def get_last_message(self):
        """Return the last successfully received message"""
...
class Candide:
    ...
    def _setup_spi(self):
        ...
        self.spi_receiver = SPIReceiver(
            clock_pin=Constants.SPI_CLOCK,
            mosi_pin=Constants.SPI_MOSI,
            miso_pin=Constants.SPI_MISO,
            cs_pin=Constants.SPI_CS,
            detect_pin=Constants.CART_DETECT
        )
    ...
    def update(self):
        ...
        try:
            ...
            if not self.spi_receiver.is_ready():
                self.spi_receiver.check_controller()
