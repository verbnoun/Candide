...
class PerformanceMonitor:
    ...
    def _calculate_load(self, audio_output):
        ...
        weighted_load = (
            (0.5 * voice_load) +           # Voice count impact
            (0.3 * self.buffer_status) +   # Buffer health
            (0.2 * (self.audio_errors / max(1, (time.monotonic() - self.last_error_time))))  # Error rate
        )
    ...
    def should_throttle(self):
        ...
        return (self.load_factor > 0.8 or 
                self.buffer_status < 0.2 or 
                self.audio_errors > 5)
...
class AudioOutputManager:
    ...
    def get_buffer_fullness(self):
        ...
        try:
            ...
            if Constants.DEBUG:
                print("[AUDIO] Buffer check failed: {0}".format(str(e)))
            self.performance.register_error()
        return 0
